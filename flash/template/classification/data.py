...
class TemplateNumpyDataSource(NumpyDataSource):
    ...
    def load_data(self, data: Tuple[np.ndarray, Sequence[Any]], dataset: Any) -> Sequence[Mapping[str, Any]]:
        ...
        :meth:`~flash.core.data.data_source.DataSource.load_data`. As we're extending the ``NumpyDataSource``, we expect
        the same ``data`` argument (in this case, a tuple containing data and corresponding target arrays).
...
class TemplateSKLearnDataSource(TemplateNumpyDataSource):
    ...
    def load_data(self, data: Bunch, dataset: Any) -> Sequence[Mapping[str, Any]]:
        """Here we're creating a fully custom :class:`~flash.core.data.data_source.DataSource` (that is, we're not going
        to treat it as one of the :class:`~flash.core.data.data_source.DefaultDataSources`) so the type of the ``data``
        argument is up to us. In this case, we want to be able to use a scikit-learn data ``Bunch`` as an input.

        On our dataset, we'll set the ``num_classes`` attribute. This is a standard practice in our classification tasks
        and ``num_classes`` will automatically be made available by the
