...
def download_ESC(data_dir, ESC10=True):
    ...
    train_df.progress_apply(
        lambda row: row_to_spec_img(base=esc50_path, filename=row['filename'], label=row['category'], split='train'),
        axis=1
    )
    valid_df.progress_apply(
        lambda row: row_to_spec_img(base=esc50_path, filename=row['filename'], label=row['category'], split='valid'),
        axis=1
    )
