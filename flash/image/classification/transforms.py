...
def default_transforms(image_size: Tuple[int, int]) -> Dict[str, Callable]:
    ...
    return {
        "pre_tensor_transform": ApplyToKeys(InputDataKeys.INPUT, T.Resize(image_size)),
        "to_tensor_transform": nn.Sequential(
            ApplyToKeys(InputDataKeys.INPUT, torchvision.transforms.ToTensor()),
            ApplyToKeys(InputDataKeys.TARGET, torch.as_tensor),
        ),
...
def train_default_transforms(image_size: Tuple[int, int]) -> Dict[str, Callable]:
    ...
    if _KORNIA_AVAILABLE and os.getenv("FLASH_TESTING", "0") != "1":
        #  Better approach as all transforms are applied on tensor directly
        transforms = {
            "post_tensor_transform": ApplyToKeys(InputDataKeys.INPUT, K.augmentation.RandomHorizontalFlip()),
        }
    else:
        transforms = {"pre_tensor_transform": ApplyToKeys(InputDataKeys.INPUT, T.RandomHorizontalFlip())}
