...
def default_transforms(image_size: Tuple[int, int]) -> Dict[str, Callable]:
    ...
    return {
        ...
        "collate": Compose([kornia_collate, ApplyToKeys(InputDataKeys.TARGET, prepare_target)]),
    }
...
def predict_default_transforms(image_size: Tuple[int, int]) -> Dict[str, Callable]:
    """During predict, we apply the default transforms only on InputDataKeys.INPUT."""
    return {
        "post_tensor_transform": nn.Sequential(
            ApplyToKeys(
                InputDataKeys.INPUT,
