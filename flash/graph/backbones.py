...
@GRAPH_BACKBONES(name="AttentiveFP", namespace="graph/classification") #todo: how to add more tasks?
def load_AttentiveFP(in_channels: int, hidden_channels: int, out_channels: int, edge_dim: int, num_layers: int, num_timesteps: int, dropout: float = 0.0):
...
@GRAPH_BACKBONES(name="DimeNet", namespace="graph/classification") #todo: how to add more tasks?
def load_DimeNet(hidden_channels, out_channels, num_blocks, num_bilinear, num_spherical, num_radial, cutoff=5.0, envelope_exponent=5, num_before_skip=1, num_after_skip=2, num_output_layers=3, act = ReLU):
    """DimeNet backbone from torch geometric"""
    return models.DimeNet(hidden_channels, out_channels, num_blocks, num_bilinear, num_spherical, num_radial, cutoff, envelope_exponent, num_before_skip, num_after_skip, num_output_layers, act)

@GRAPH_BACKBONES(name="GraphUNet", namespace="graph/classification") #todo: how to add more tasks?
...
@GRAPH_BACKBONES(name="RENet", namespace="graph/classification") #todo: how to add more tasks?
...
@GRAPH_BACKBONES(name="SchNet", namespace="graph/classification") #todo: how to add more tasks?
def load_SchNet(hidden_channels=128, num_filters=128, num_interactions=6, num_gaussians=50, cutoff=10.0, readout='add', dipole=False, mean=None, std=None, atomref=None):
    """SchNet backbone from torch geometric"""
    return models.SchNet(hidden_channels, num_filters, num_interactions, num_gaussians, cutoff, readout, dipole, mean, std, atomref)

@GRAPH_BACKBONES(name="SignedGCM", namespace="graph/classification") #todo: how to add more tasks?
...
@GRAPH_BACKBONES(name="LabelPropagation", namespace="graph/classification") #todo: how to add more tasks?
...
@GRAPH_BACKBONES(name="ARGVA", namespace="graph/classification") #todo: how to add more tasks?
...
@GRAPH_BACKBONES(name="ARGA", namespace="graph/classification") #todo: how to add more tasks?
...
@GRAPH_BACKBONES(name="GAE", namespace="graph/classification") #todo: how to add more tasks?
def load_GAE(encoder, decoder=None):
    """GAE backbone from torch geometric"""
    return models.GAE(encoder, decoder)

@GRAPH_BACKBONES(name="VGAE", namespace="graph/classification") #todo: how to add more tasks?
