...
class AutoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if self.load_data is not None:
            self._processed_data = self._call_load_data(data)
        else:
            self._processed_data = self.data

        if self.data_pipeline is not None and self._running_stage is not None:
            self._setup(self.running_stage)
    ...
    def _setup(self, stage: RunningStage):
        ...
        if self.data_pipeline is not None and self.load_data is None and self.load_sample is None:
        ...
        if self.load_data is not None and old_load_data != self.load_data.__code__:
            if old_load_data is not None:
                ...
                self._processed_data = self._call_load_data(self.data)

    def __getitem__(self, index: int) -> Any:
        if self.load_sample is not None:
            return self._call_load_sample(self._processed_data[index])
        else:
