...
if _PL_GREATER_EQUAL_1_5_0:
    from pytorch_lightning.utilities.cli import *  # noqa: F403, F401
    from pytorch_lightning.utilities.cli import _get_short_description
else:
    import inspect
    import os
    import sys
        ...
        def __call__(self, cls: Type, key: Optional[str] = None, override: bool = False) -> Type:
            ...
            if key in self and not override:
                raise MisconfigurationException(
                    f"'{key}' is already present in the registry. HINT: Use `override=True`."
                )
            self[key] = cls
            return cls
    ...
    class LightningArgumentParser(ArgumentParser):
        ...
        def _convert_argv_issue_84(classes: Tuple[Type, ...], nested_key: str, argv: List[str]) -> List[str]:
            ...
            if not argv_class:
                ...
                init_args = {
                    k[len(init_args_key) + 1 :]: v for k, v in passed_args.items() if k.startswith(init_args_key)
                }
    ...
    class LightningCLI:
        ...
        def _add_subcommands(self, parser: LightningArgumentParser, **kwargs: Any) -> None:
            ...
            for subcommand in self.subcommands():
                subcommand_parser = self._prepare_subcommand_parser(
                    trainer_class, subcommand, **kwargs.get(subcommand, {})
                )
        ...
        def _add_configure_optimizers_method_to_model(self, subcommand: Optional[str]) -> None:
            ...
            if len(optimizers) > 1 or len(lr_schedulers) > 1:
                raise MisconfigurationException(
                    f"`{self.__class__.__name__}.add_configure_optimizers_method_to_model` expects at most one"
                    f" optimizer and one lr_scheduler to be 'AUTOMATIC', but found {optimizers+lr_schedulers}."
                    " In this case the user is expected to link the argument groups and implement"
                    " `configure_optimizers`, see "
