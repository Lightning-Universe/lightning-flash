...
class FlashTransform(Properties):
    ...
    def _resolve_transforms(self, running_stage: RunningStage) -> Optional[Dict[str, Callable]]:
        ...
        resolved_function = getattr(
            self, DataPipeline._resolve_function_hierarchy("configure_transforms", self, running_stage, FlashTransform)
        )
    ...
    def _check_transforms(
        ...
        if not isinstance(transform, Dict):
            raise MisconfigurationException(
                ...
                f"Here are the available keys for your transforms: {[v for v in TransformPlacement]}."
            )

        keys_diff = set(transform.keys()).difference([v for v in TransformPlacement])
    ...
    def __getitem__(self, placement: TransformPlacement) -> Callable:
