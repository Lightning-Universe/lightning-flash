...
class IceVisionAdapter(Adapter):
    ...
    def _collate_fn(collate_fn, samples, metadata: Optional[List[Dict[str, Any]]] = None):
        ...
        return {
            InputDataKeys.INPUT: collate_fn(
                [to_icevision_record({**sample, InputDataKeys.METADATA: m}) for sample, m in zip(samples, metadata)]
            ),
    ...
    def training_step(self, batch, batch_idx) -> Any:
        return self.icevision_adapter.training_step(batch[InputDataKeys.INPUT], batch_idx)

    def validation_step(self, batch, batch_idx):
        return self.icevision_adapter.validation_step(batch[InputDataKeys.INPUT], batch_idx)

    def test_step(self, batch, batch_idx):
        return self.icevision_adapter.validation_step(batch[InputDataKeys.INPUT], batch_idx)

    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        batch[InputDataKeys.PREDS] = self(batch[InputDataKeys.INPUT])
        return batch
