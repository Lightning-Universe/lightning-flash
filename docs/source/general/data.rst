...
.. list-table:: Terminology
   ...
   * - :class:`~flash.core.data.data_pipeline.DataPipeline`
     - The :class:`~flash.core.data.data_pipeline.DataPipeline` is Flash internal object to manage :class:`~flash.core.data.Deserializer`, :class:`~flash.core.data.io.input.BaseInput`, :class:`~flash.core.data.process.Preprocess`, :class:`~flash.core.data.process.Postprocess`, and :class:`~flash.core.data.process.Serializer` objects.
   * - :class:`~flash.core.data.io.input.BaseInput`
     - The :class:`~flash.core.data.io.input.BaseInput` provides :meth:`~flash.core.data.io.input.BaseInput.load_data` and :meth:`~flash.core.data.io.input.BaseInput.load_sample` hooks for creating data sets from metadata (such as folder names).
...
The :class:`~flash.core.data.io.input.BaseInput` class can be used to generate data sets from multiple sources (e.g. folders, numpy, etc.), that can then all be transformed in the same way.
...
In each ``from_*`` method, the :class:`~flash.core.data.data_module.DataModule` internally retrieves the correct :class:`~flash.core.data.io.input.BaseInput` to use from the :class:`~flash.core.data.process.Preprocess`.
Flash :class:`~flash.core.data.auto_dataset.AutoDataset` instances are created from the :class:`~flash.core.data.io.input.BaseInput` for train, val, test, and predict.
...
The example below shows a very simple ``ImageClassificationPreprocess`` with a single ``ImageClassificationFoldersInput`` and an ``ImageClassificationDataModule``.
...
.. code-block:: python
    ...
    from typing import Any, Dict
    from flash.core.data.io.base_input import BaseInput
    from flash.core.data.io.input import InputDataKeys


    class ImageClassificationFoldersInput(BaseInput):
...
We get the ``from_folders`` classmethod for free as we've registered a ``InputFormat.FOLDERS`` data source in our ``ImageClassificationPreprocess``.
...
.. note::
    The :meth:`~flash.core.data.io.input.BaseInput.load_data` and
    :meth:`~flash.core.data.io.input.BaseInput.load_sample` will be used to generate an
...
.. code-block:: python
    ...
    class AutoDataset:
        def __init__(
            self,
            data: List[Any],  # output of `BaseInput.load_data`
            data_source: BaseInput,
