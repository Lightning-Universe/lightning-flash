...
def test_classification_serializers():
    ...
    assert torch.allclose(torch.tensor(FiftyOneLabels(
        store_logits=True).serialize(example_output).logits), example_output)
    assert torch.allclose(torch.tensor(Probabilities().serialize(example_output)), torch.softmax(example_output, -1))
    assert torch.allclose(torch.tensor(FiftyOneLabels().serialize(
        example_output).confidence), torch.softmax(example_output, -1)[-1])
...
def test_classification_serializers_multi_label():
    ...
    assert torch.allclose(torch.tensor(FiftyOneLabels(
        store_logits=True, multi_label=True).serialize(example_output).logits), example_output)
    ...
    assert [c.label for c in FiftyOneLabels(labels, multi_label=True).serialize(
        example_output).classifications] == ['class_2', 'class_3']
