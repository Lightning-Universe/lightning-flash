...
def test_utility_load():
    data = [{
        ...
        "data": {
            "image": "/data/upload/Highway20030201_1002591.jpg"
        },
        "meta": {},
    ...
    assert ds[3] == set(["image"])
    assert ds[2] == set(["Road", "Car", "Obstacle"])
    assert len(ds[1]) == 0
    assert len(ds[0]) == 5
    ds_multi = LabelStudioDataSource._load_json_data(data=data, data_folder=".", multi_label=True)
    assert ds_multi[3] == set(["image"])
    assert ds_multi[2] == set(["Road", "Car", "Obstacle"])
