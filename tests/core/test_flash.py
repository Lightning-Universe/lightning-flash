...
def test_compute_dict():
    ...
    metric_vals = Flash.compute_dict(
        metrics, y=torch.zeros(1,), y_hat=torch.zeros(1,), prefix="", sep=""
    )
...
def test_compute_metrics():
    ...
    metric_vals = model.compute_metrics(
        y=torch.zeros(1,), y_hat=torch.zeros(1,), prefix="", sep=""
    )
...
def test_train_step():
    ...
    for k in expected_keys:
        assert f"step_train/{k}" in result
        assert f"epoch_train/{k}" in result
...
def _val_test_step(prefix: str):
    ...
    for k in expected_keys:
        assert f"step_{prefix}/{k}" in result
        assert f"epoch_{prefix}/{k}" in result
