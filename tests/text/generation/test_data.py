...
from flash.text.generation import TextGenerationData
from flash.text.generation.data import (
    TextGenerationCSVDataSource,
    TextGenerationDataSource,
    TextGenerationFileDataSource,
    TextGenerationJSONDataSource,
    TextGenerationSentencesDataSource,
)
...
TEST_BACKBONE = ""  # super small model for testing
...
def test_from_csv(tmpdir):
    ...
    dm = TextGenerationData.from_csv("sentence", "label", backbone=TEST_BACKBONE, train_file=csv_path, batch_size=1)
...
def test_test_valid(tmpdir):
    csv_path = csv_data(tmpdir)
    dm = TextGenerationData.from_csv(
        "sentence",
        "label",
...
def test_from_json(tmpdir):
    ...
    dm = TextGenerationData.from_json("sentence", "lab", backbone=TEST_BACKBONE, train_file=json_path, batch_size=1)
    batch = next(iter(dm.train_dataloader()))
    assert batch["labels"].item() in [0, 1]
    assert "input_ids" in batch
...
def test_from_json_with_field(tmpdir):
    ...
    dm = TextGenerationData.from_json(
...
def test_text_module_not_found_error():
    with pytest.raises(ModuleNotFoundError, match="[text]"):
        TextGenerationData.from_json("sentence", "lab", backbone=TEST_BACKBONE, train_file="", batch_size=1)
...
@pytest.mark.parametrize(
    ...
    [
        (TextGenerationDataSource, {}),
        (TextGenerationFileDataSource, {"filetype": "csv"}),
        (TextGenerationCSVDataSource, {}),
        (TextGenerationJSONDataSource, {}),
        (TextGenerationSentencesDataSource, {}),
    ],
